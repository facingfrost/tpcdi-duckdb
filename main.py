...
def run_tasks(file_path,scale_factor):
    '''file_path: str, scale_factor: int'''
    file_path = file_path + '/sf_' + str(scale_factor)
    ...
    con = duckdb.connect('tpcdi_'+str(scale_factor)+'.db')
    ...
    # Task8,9,10,11 - Direct load master.tradetype, master.statustype, master.taxrate, master.industry
    ...
    # Task16,17,18,19 - Transform & load master.dimbroker, prospect, customer, dimessages_customer
    ...
    # Task23 - Transform & load master.dimtrade, divided into 2 steps
    ...
    df = pd.DataFrame(res)
    df.to_csv("./results/result_"+str(scale_factor)+".csv", index=False)

def main():
    run_tasks(file_path="/Users/wanglinhan/Desktop/BDMA/ULB/INFO-H419/Project/TPCDI/tpcdi_duckdb/data",scale_factor="3")
